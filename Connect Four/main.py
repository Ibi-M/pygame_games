...
def draw(won):
    ...
    instructions = INSTRUCTIONS_FONT.render("Press 1-7 to drop your piece in a column", True, BLACK)
    ...
    if not won:
        turn = TITLE_FONT.render("Player " + str(player + 1) + "'s Turn", True, COLORS[player])
        screen.blit(turn, (170,600))
    elif won:
        WIN_FONT.set_underline(True)
        final = WIN_FONT.render(f"Player {player + 1} WINS!", True, COLORS[player])
...
def win(p): 
    ...
    for r in range(6):
        for c in range(4):
            if taken[r][c] == p and taken[r][c + 1] == p and taken[r][c + 2] == p and taken[r][c + 3] == p:
                return True
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                valid_numbers = [pygame.K_1, pygame.K_2, pygame.K_3, pygame.K_4, pygame.K_5, pygame.K_6, pygame.K_7]
                if event.key in valid_numbers:
                    column = event.key - pygame.K_1 
                    print(f"Player {player + 1} wants to drop in column {column}")
